...
int main(void)
{
	...
	//double wid[]={0.06, 0.30, 0.08, 0.035, 0.23,0.30, 0.3, 0.65}; //width of random walk for n=300
	double wid[]={0.045, 0.23, 0.07, 0.031, 0.20, 0.23, 0.27, 0.6}; //width of random walk for n=500
	int i, j;
	...
	for (j=0; j<sim; j++)
	{
		for(i=0; i<n; i++)
		{
			...
			e3[i] = -log(1-((double)rand()/((double)RAND_MAX+1)))*e3sd - e3sd;
			Y[i] = b0t + b1t*W[i] + b2t*U[i] + e3[i];
			eC1[i] = -log(1-((double)rand()/((double)RAND_MAX+1)))*e3sd - e3sd;
			C1[i] = b0t + b1t*W[i] + b2t*U[i] + eC1[i];
			eC2[i] = -log(1-((double)rand()/((double)RAND_MAX+1)))*e3sd - e3sd;
