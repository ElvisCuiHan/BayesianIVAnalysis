...
#dm_filtered = read.delim(file = "0322_DPMIV_UKB_less_3_6.dat", sep=" ", header=T)
#dm_filtered = read.delim(file = "0322_DPMIV_UKB.dat", sep=" ", header=T)
dm_filtered = read.delim(file = "0330_DPMIV_test_data_eth1_male_subsetsnp_colnames.txt", sep=" ", header=T)
#ca = read_delim(file="0325DPMIV_Filtered_with_SNP.txt")[, c(1:4, 12:280, 6:10)]
#ca = read_delim("0321_DPMIV_UKB_5U.dat")
#colnames(dm_filtered) = colnames(ca)[-c(15,16,18,21,23)]
#colnames(dm_filtered) = colnames(ca)[-279]
...
mod2 = ic_par(cbind(exp(L), exp(R)) ~ LOG_SBP  + AGE_RECRUITMENT +
                CHOLESTEROL + BMI + SMOKE + MET, model="aft", 
              data = dm_filtered, dist = "lnorm")
summary(mod2)
...
### Two Stage AFT Model
# Noisy surrogate
X = as.matrix(dm_filtered$LOG_SBP)
# Instruments
G = as.matrix(dm_filtered[, c(5:19)])
U = as.matrix(dm_filtered[, c("AGE_RECRUITMENT","CHOLESTEROL","BMI","SMOKE","MET")])
fit1=lm(X~G+U, na.action=na.exclude);
pred=predict(fit1);
fit2=ic_par(cbind(exp(L), exp(R)) ~ pred + AGE_RECRUITMENT +
              CHOLESTEROL + BMI + SMOKE + MET, model="aft", 
            data = dm_filtered, dist = "lnorm")
summary(fit2)
...
mod_null = survreg(Surv(time=exp(L), time2=exp(R), type="interval2")~LOG_SBP + AGE_RECRUITMENT +
                     CHOLESTEROL + BMI + SMOKE + MET, 
