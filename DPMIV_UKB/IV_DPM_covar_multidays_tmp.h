...
void IV_DPM_MH_covar_tmp(double *X, double *L, double *R, int *d, int n, int kG, int kU, double **G, double **U, int ch, 
					 ...
					 char tstamp[])  
{
	...
	int i, j, k, ii, k2, k3, npar=7+kG+2*kU;  
	...
	int *ci, *freqcl, *dc;  
	double *Xc, *Lc, *Rc, *Tc, **Gc, **Uc; // to store data for cluster c;
	...
	long double logL, logLn, logA, u; //log-likelihoods; log acceptance prob; uniform(0,1)
	...
	FILE *temp_b2, *temp_a1, *temp_a2;
	FILE *ind_result; // to save individual level posterior samples
	...
	b2r = (double *) malloc(kU*nsave*sizeof(double));
	a1r = (double *) malloc(kG*nsave*sizeof(double));
	a2r = (double *) malloc(kU*nsave*sizeof(double));
	vr = (double *) malloc(nsave*sizeof(double));
	nclr = (int *) malloc(nsave*sizeof(int));
	
	for (i=0; i<npar; i++) accept[i]=0;

	//printf("run 40 times for different initial values >>>>>>>>>>>>>>>>>>>>>>>\n");
	printf("run with SNP IV >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
...
//temp=fopen("temp.txt", "w");
	sprintf(fname, "%s_temp.txt", tstamp);
	temp=fopen(fname, "w");
	fprintf(temp, "b1	b2	a1	a2	v	ncl\n");
	fprintf(temp, "%lf %lf %lf %lf %lf %d\n", b1, init_b2[0], init_a1[0], init_a2[0], v, ncl);
	fclose(temp);

/* 	sprintf(fname, "%s_trace_a1.txt", tstamp);
	temp_a1=fopen(fname, "w");
	for (i=0; i<kG; i++) { 
		fprintf(temp_a1, "%lf ", init_a1[i]);
	}
	fprintf(temp_a1, "\n");
	fclose(temp_a1);

	sprintf(fname, "%s_trace_b2.txt", tstamp);
	temp_b2=fopen(fname, "w");
	sprintf(fname, "%s_trace_a2.txt", tstamp);
	temp_a2=fopen(fname, "w");
	for (i=0; i<kU; i++) {
		fprintf(temp_b2, "%lf ", init_b2[i]);
		fprintf(temp_a2, "%lf ", init_a2[i]);
	}
	fprintf(temp_b2, "\n");
	fprintf(temp_a2, "\n");
	fclose(temp_b2);
    ...
    sprintf(fname, "%s_ind_result.txt", tstamp);
	ind_result=fopen(fname, "w");
	fprintf(ind_result, "mu1  sig1  mu2  sig2  rho\n");
	fclose(ind_result);
	...
	for (j=1; j<=ch; j++) 
	{
		...
		if (j == ch) {
			sprintf(fname, "%s_ind_result.txt", tstamp);
			ind_result=fopen(fname, "a");
			for (i=0; i<n; i++)	fprintf(temp, "%f  %f  %f  %f  %f\n", mu1[i], sig1[i], mu2[i], sig1[i], rho[i]);  
			fclose(ind_result);
		}

		// Log-likelihood;
		logL = logL_IV_DPM_UKB(L, R, d, X, n, G, kG, U, kU, b1, a1, a2, b2, mu1, sig1, mu2, sig2, rho);
		...
		b1n = runif(b1-w, b1+w);
		logLn = logL_IV_DPM_UKB(L, R, d, X, n, G, kG, U, kU, b1n, a1, a2, b2, mu1, sig1, mu2, sig2, rho);
		...
		for (k=0; k<kG; k++){
			...
			logLn = logL_IV_DPM_UKB(L, R, d, X, n, G, kG, U, kU, b1, a1n, a2, b2, mu1, sig1, mu2, sig2, rho);
			logA = logLn - logL + ((a1[k]-mu_p)*(a1[k]-mu_p)-(a1n[k]-mu_p)*(a1n[k]-mu_p))/sig_p/2.0;
			if(logA>=0) acp=1; 
			else { 
				u = log((double) rand() / ((double)RAND_MAX+1)) ; 
				if (logA>=u) acp=1; 
				else acp=0; 
			}	
			if (acp==1) { 
		...
		for (k=0; k<kU; k++){
			...
			logLn = logL_IV_DPM_UKB(L, R, d, X, n, G, kG, U, kU, b1, a1, a2n, b2, mu1, sig1, mu2, sig2, rho);
		...
		for (k=0; k<kU; k++){
			...
			logLn = logL_IV_DPM_UKB(L, R, d, X, n, G, kG, U, kU, b1, a1, a2, b2n, mu1, sig1, mu2, sig2, rho);
		...
		for (i=0; i<n; i++)	// update the cluster status of each subject
		{
			...
			if (single==0)	// subject i is not a singleton
			{
				for (k=ncl; k<ncl+m; k++)  // generate m auxiliary parameters
				{
					...
					//mu2_c[k] = LDMAX(LDMIN(mu2_c[k], 300), -10);
			...
			for (k=0; k < (ncl-m); k++)  // ncl-m equals k-minus in Neal (2000)
				pclust[k] = freqcl[k] * L_IV_ind_UKB(L[i], R[i], d[i], X[i], G[i], kG, U[i], kU, b1, a1, a2, b2, mu1_c[k], sig1_c[k], mu2_c[k], sig2_c[k], rho_c[k]) ;
			for (k=ncl-m; k<ncl; k++)
				pclust[k] = v/m * L_IV_ind_UKB(L[i], R[i], d[i], X[i], G[i], kG, U[i], kU, b1, a1, a2, b2, mu1_c[k], sig1_c[k], mu2_c[k], sig2_c[k], rho_c[k]) ;
		...
		for (k=0; k<ncl; k++)
		{
			...
			// Log-likelihood;
			logL = logL_IV_normal_UKB(Lc, Rc, dc, Xc, k2, Gc, kG, Uc, kU, mu1_c[k], a1, a2, sig1_c[k], mu2_c[k], b1, b2, sig2_c[k], rho_c[k]); 

			// Metropolis-Hastings sampling for mu1_c 
			w=width_c[0];
			mu_p = prior_c1[0]; sig_p = prior_c2[0]; 
			mu1_cn = runif(mu1_c[k]-w, mu1_c[k]+w);
			logLn = logL_IV_normal_UKB(Lc, Rc, dc, Xc, k2, Gc, kG, Uc, kU, mu1_cn, a1, a2, sig1_c[k], mu2_c[k], b1, b2, sig2_c[k], rho_c[k]); 
			...
			logLn = logL_IV_normal_UKB(Lc, Rc, dc, Xc, k2, Gc, kG, Uc, kU, mu1_c[k], a1, a2, sig1_cn, mu2_c[k], b1, b2, sig2_c[k], rho_c[k]); 
			...
			//mu2_cn = LDMAX(LDMIN(mu2_cn, 120), -1);
			logLn = logL_IV_normal_UKB(Lc, Rc, dc, Xc, k2, Gc, kG, Uc, kU, mu1_c[k], a1, a2, sig1_c[k], mu2_cn, b1, b2, sig2_c[k], rho_c[k]); 
			...
			logLn = logL_IV_normal_UKB(Lc, Rc, dc, Xc, k2, Gc, kG, Uc, kU, mu1_c[k], a1, a2, sig1_c[k], mu2_c[k], b1, b2, sig2_cn, rho_c[k]); 
			...
			logLn = logL_IV_normal_UKB(Lc, Rc, dc, Xc, k2, Gc, kG, Uc, kU, mu1_c[k], a1, a2, sig1_c[k], mu2_c[k], b1, b2, sig2_c[k], rho_cn); 
		...
		if (i_tmp==nsave) {
			...
			printf("     b1: %f\n", (double)accept[0]/j);
			printf("     a1: "); for(i=0; i<LDMIN(kG, NN); i++) printf("%f  ", (double)accept[1+i]/j); printf("\n");
			printf("     a2: "); for(i=0; i<LDMIN(kU, NN); i++) printf("%f  ", (double)accept[1+kG+i]/j); printf("\n");
			printf("     b2: "); for(i=0; i<LDMIN(kU, NN); i++) printf("%f  ", (double)accept[1+kG+kU+i]/j); printf("\n");
			printf("theta_c: "); for(i=0; i<5; i++) printf("%f  ", (double)accept[1+kG+2*kU+i]/s_clust); printf("\n");
			printf("      v: %f\n", (double)accept[6+kG+2*kU]/j); 
			printf("average num of clust: %f\n", (double)s_clust/j);
			printf("...............................................................\n");
			printf("b1, b2, a1, a2: %f, %f, %f, %f\n", b1, b2[0], a1[0], a2[0]);
		
			// save the temp results
			sprintf(fname, "%s_temp.txt", tstamp);
			temp=fopen(fname, "a");
			//sprintf(fname, "%s_trace_a1.txt", tstamp);
			//temp_a1=fopen(fname, "a");
			//sprintf(fname, "%s_trace_b2.txt", tstamp);
			//temp_b2=fopen(fname, "a");
			//sprintf(fname, "%s_trace_a2.txt", tstamp);
			//temp_a2=fopen(fname, "a");
			for (i=0; i<nsave; i++)	{
				fprintf(temp, "%lf %lf %lf %lf %lf %d\n", b1r[i], b2r[i*kU], a1r[i*kG], a2r[i*kU], vr[i], nclr[i]);  

/* 				for (ii=0; ii<kG; ii++) { 
					fprintf(temp_a1, "%lf ", a1r[i*kG+ii]);
				}
				fprintf(temp_a1, "\n");

				for (ii=0; ii<kU; ii++) {
					fprintf(temp_b2, "%lf ", b2r[i*kU+ii]);
					fprintf(temp_a2, "%lf ", a2r[i*kU+ii]);
				}
				fprintf(temp_b2, "\n");
				fprintf(temp_a2, "\n"); */
			...
			sprintf(fname, "%s_temp2_last.txt", tstamp);
			temp=fopen(fname, "w");
			fprintf(temp, "%lf\n", b1);
 			for(i=0; i<kG; i++) fprintf(temp, "%lf ", a1[i]); fprintf(temp,"\n"); 
			for(i=0; i<kU; i++) fprintf(temp, "%lf ", a2[i]); fprintf(temp,"\n");
			for(i=0; i<kU; i++) fprintf(temp, "%lf ", b2[i]); fprintf(temp,"\n"); 
			fprintf(temp, "%lf\n", v);
			fprintf(temp, "%d\n", ncl); 
			for(i=0; i<ncl; i++) fprintf(temp, "%lf %lf %lf %lf %lf %d\n", mu1_c[i], sig1_c[i], mu2_c[i], sig2_c[i], rho_c[i], freqcl[i]);		
	...
	printf("acceptance rate:\n");
	printf("     b1: %f\n", (double)accept[0]/ch);
	printf("     a1: "); for(i=0; i<LDMIN(kG, NN); i++) printf("%f  ", (double)accept[1+i]/ch); printf("\n");
	printf("     a2: "); for(i=0; i<LDMIN(kU, NN); i++) printf("%f  ", (double)accept[1+kG+i]/ch); printf("\n");
	printf("     b2: "); for(i=0; i<LDMIN(kU, NN); i++) printf("%f  ", (double)accept[1+kG+kU+i]/ch); printf("\n");
	printf("theta_c: "); for(i=0; i<5; i++) printf("%f  ", (double)accept[1+kG+2*kU+i]/s_clust); printf("\n");
