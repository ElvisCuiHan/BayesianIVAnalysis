...
int main(void)
{
	...
	//double wid[]={0.06, 0.25, 0.08, 0.035, 0.25, 0.31, 0.35, 0.42}; //width of random walk for n=300
	double wid[]={0.04, 0.21, 0.06, 0.030, 0.20, 0.27, 0.25, 0.37}; //width of random walk for n=500
	...
	double prior_v[]={0.01, 2.9, 0.8}; // (v_low, v_up, v_w) in prior of v: proportional to ((v_up-v)/(v_up-v_low))^v_w  (Conley et al. 2008)
	...
	for (j=0; j<sim; j++)
	{
		for(i=0; i<n; i++)
		{
			...
			clust = ((double) rand() / ((double)RAND_MAX+1));
			if (clust<=.5) clust=1; else clust=2;
			e3[i] = ((-sqrt(8.0/9.0)+gasdev(&idum)/3.0)*(clust==1) + (sqrt(8.0/9.0)+gasdev(&idum)/3.0)*(clust==2))*e3sd;
			Y[i] = b0t + b1t*W[i] + b2t*U[i] + e3[i];
			clust = ((double) rand() / ((double)RAND_MAX+1));
			if (clust<=.5) clust=1; else clust=2;
			eC1[i] = ((-sqrt(8.0/9.0)+gasdev(&idum)/3.0)*(clust==1) + (sqrt(8.0/9.0)+gasdev(&idum)/3.0)*(clust==2))*e3sd;
			C1[i] = b0t + b1t*W[i] + b2t*U[i] + eC1[i];
			clust = ((double) rand() / ((double)RAND_MAX+1));
			if (clust<=.5) clust=1; else clust=2;
			eC2[i] = ((-sqrt(8.0/9.0)+gasdev(&idum)/3.0)*(clust==1) + (sqrt(8.0/9.0)+gasdev(&idum)/3.0)*(clust==2))*e3sd;
